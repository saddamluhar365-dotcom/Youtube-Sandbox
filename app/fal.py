...
class FalVideoClient:
    ...
    def wait(self, request_id: str) -> ProviderVideoResult:
        ...
        try:
            lease = self.pool.acquire_slot(slot_id)
        except (KeyError, RuntimeError) as exc:
            raise RuntimeError("fal.ai bound credential is temporarily unavailable") from exc
        ...
        try:
            while time.time() < deadline:
                response = httpx.get(f"{base}/status", headers={"Authorization": f"Key {lease.secret}"}, timeout=30)
                if response.status_code in (401, 403):
                ...
                if status == "COMPLETED":
                    result = httpx.get(f"{base}", headers={"Authorization": f"Key {lease.secret}"}, timeout=30)
